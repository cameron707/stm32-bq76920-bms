...
> **Note:** This HARA follows ISO 26262-3 methodology for hazard identification and ASIL determination. ASIL is calculated per Table 4 based on Severity (S), Exposure (E), and Controllability (C). For borderline classifications (QM vs ASIL A), the more conservative ASIL A is selected to ensure safety margin.
...
| HZD-006 | I2C communication failure between STM32 and BQ76920 | Normal operation, any condition | S1 | E3 | C2 | QM | Detect I2C failure and disable FETs within 100ms | Protection cannot work if BQ76920 can't be read. QM justified as severity is minor (S1). |
...
| HZD-009 | BQ76920 enters SHIP mode (low power) | Normal operation, idle state | S1 | E2 | C2 | QM | Periodically verify BQ76920 is awake and communicating | SHIP mode disables protection. Software must detect. QM justified as severity is minor (S1). |
...
| HZD-006 | S1 | E3 | C2 | QM | S1 (minor injury) → QM recommended per ISO 26262-3 Table 4; severity too low for ASIL classification |
...
| HZD-009 | S1 | E2 | C2 | QM | S1 (minor injury) → QM recommended per ISO 26262-3 Table 4 |
...
| HZD-006 | Detect I2C failure and disable FETs | <a href="SRS.md#hwi-01">HWI-01</a> | TC-SYS-I2C-001 |
